// Copyright 2017 Palantir Technologies. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

package encryptedconfigvalue_test

import (
	"testing"

	"github.com/palantir/go-encrypted-config-value/encryptedconfigvalue"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

const (
	testPlaintext = "plaintext"

	testAESEncryptedValKey encryptedconfigvalue.SerializedKeyWithType    = "AES:LICx0yKzQm5a6IE13aJ3xOsRv+8AujqHocTFI4yk4Jw="
	testAESEncryptedVal    encryptedconfigvalue.SerializedEncryptedValue = "enc:eyJ0eXBlIjoiQUVTIiwibW9kZSI6IkdDTSIsImNpcGhlcnRleHQiOiJNOTRrSXlvYTUrMloiLCJpdiI6InVBR3FSbFA5d2l6cGRCMHoiLCJ0YWciOiJBQ1N1ekR3VFVMb21zanhwRk1rWUtBPT0ifQ=="

	testRSAEncryptedValPrivKey encryptedconfigvalue.SerializedKeyWithType    = "RSA-PRIV:"
	testRSAEncryptedVal        encryptedconfigvalue.SerializedEncryptedValue = "enc:"

	javaPlaintext                                                           = "my secret. I don't want anyone to know this"
	javaAESKey                encryptedconfigvalue.SerializedKeyWithType    = "AES:rqrvWpLld+wKLOyxJYxQVg=="
	javaLegacyAESEncryptedVal encryptedconfigvalue.SerializedEncryptedValue = "enc:QjR4AHIYoIzvjEHf53XETM3QYnCl1mgFYC51Q7x4ebwM+h3PHVqSt/1un/+KvpJ2mZfMH0tifu+htRVxEPyXmt88lyKB83NpesNJEoLFLL+wBWCkppaLRuc/1w=="

	javaRSALegacyPrivKey      encryptedconfigvalue.SerializedKeyWithType    = "RSA:"
	javaLegacyRSAEncryptedVal encryptedconfigvalue.SerializedEncryptedValue = "enc:"
)

func TestDecryptEncryptedValue(t *testing.T) {
	for i, currCase := range []struct {
		name          string
		plaintext     string
		decryptionKey encryptedconfigvalue.SerializedKeyWithType
		encryptedVal  encryptedconfigvalue.SerializedEncryptedValue
	}{
		{
			name:          "AES",
			plaintext:     testPlaintext,
			decryptionKey: testAESEncryptedValKey,
			encryptedVal:  testAESEncryptedVal,
		},
		{
			name:          "AES legacy",
			plaintext:     javaPlaintext,
			decryptionKey: javaAESKey,
			encryptedVal:  javaLegacyAESEncryptedVal,
		},
		{
			name:          "RSA",
			plaintext:     testPlaintext,
			decryptionKey: testRSAEncryptedValPrivKey,
			encryptedVal:  testRSAEncryptedVal,
		},
		{
			name:          "RSA legacy",
			plaintext:     javaPlaintext,
			decryptionKey: javaRSALegacyPrivKey,
			encryptedVal:  javaLegacyRSAEncryptedVal,
		},
	} {
		decKey, err := encryptedconfigvalue.NewKeyWithTypeFromSerialized(currCase.decryptionKey)
		require.NoError(t, err, "Case %d: %s", i, currCase.name)

		ev, err := encryptedconfigvalue.NewEncryptedValueFromSerialized(currCase.encryptedVal)
		require.NoError(t, err, "Case %d: %s", i, currCase.name)

		decrypted, err := ev.Decrypt(decKey)
		require.NoError(t, err, "Case %d: %s", i, currCase.name)

		assert.Equal(t, currCase.plaintext, decrypted, "Case %d: %s", i, currCase.name)
	}
}
